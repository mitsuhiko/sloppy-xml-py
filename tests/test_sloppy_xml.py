...
from sloppy_xml import (
    StartElement, EndElement, Text, Comment, ProcessingInstruction, 
    EntityRef, ParseError, XMLEvent, ParseOptions, RecoveryStrategy,
    TreeBuilder, ElementTreeBuilder, HTML_ENTITIES
)


class TestBasicFunctionality:
    """Test basic XML parsing functionality with well-formed documents."""
...
class TestStreamParsing:
    """Test streaming XML parsing functionality."""
    ...
    def test_stream_events_order(self):
        ...
        expected = ["Comment", "StartElement", "Text", "EndElement", "ProcessingInstruction"]
    ...
    <child>text</child>
        ...
        child_events = [e for e in events if isinstance(e, StartElement) and e.name == "child"]
    ...
    def test_streaming_large_input(self):
        ...
        large_xml = "<root>" + "".join(f"<item{i}>content{i}</item{i}>" for i in range(1000)) + "</root>"
    ...
    def test_legacy_parameters(self):
        ...
        assert "text & more" in text_event.content or "text &amp; more" in text_event.content


class TestTreeBuilding:
    """Test XML tree construction functionality."""
    ...
    def test_custom_tree_builder(self):
        """Test using custom tree builder."""
        class MockTreeBuilder(TreeBuilder):
            def __init__(self):
                self.events = []
            
            def start_element(self, event): self.events.append(("start", event.name))
            def end_element(self, event): self.events.append(("end", event.name))
            def text(self, event): self.events.append(("text", event.content))
            def comment(self, event): self.events.append(("comment", event.content))
            def processing_instruction(self, event): self.events.append(("pi", event.target))
            def entity_ref(self, event): self.events.append(("entity", event.name))
            def parse_error(self, event): self.events.append(("error", event.message))
            def get_root(self): return self.events
    ...
    def test_tree_parse_from_events(self):
...
class TestEntityResolution:
    ...
    def test_extended_html_entities(self):
    ...
    def test_entity_resolution_disabled(self):
    ...
    def test_malformed_numeric_entities(self):
...
class TestComments:
    ...
    def test_nested_comment_chars(self):
...
class TestProcessingInstructions:
    """Test XML processing instruction handling."""
...
class TestErrorRecovery:
    """Test error recovery scenarios with malformed XML."""
    ...
    def test_unescaped_characters(self):
    ...
    def test_recovery_strategies(self):
        ...
        for strategy in strategies:
            ...
            if strategy == RecoveryStrategy.AGGRESSIVE:
                ...
                element_events = [e for e in events if isinstance(e, (StartElement, EndElement))]
...
class TestNamespaces:
    ...
    def test_namespace_disabled(self):
        ...
        xml = '<ns:root xmlns:ns="http://example.com"><ns:child>content</ns:child></ns:root>'
...
class TestPerformance:
    ...
    def test_large_document_performance(self):
        ...
        num_elements = 5000
        xml_parts = ["<root>"]
        xml_parts.extend(f"<item{i} id='{i}'>Content for item {i}</item{i}>" for i in range(num_elements))
    ...
    def test_deep_nesting_performance(self):
        ...
        start_time = time.time()
        events = list(sloppy_xml.stream_parse(deep_xml, options=ParseOptions(max_depth=600)))
        parse_time = time.time() - start_time
    ...
    def test_memory_usage_streaming(self):
        ...
        large_xml = "<root>" + "".join(f"<item{i}>data</item{i}>" for i in range(1000)) + "</root>"
    ...
    def test_entity_heavy_performance(self):
...
class TestRealWorldScenarios:
    """Test real-world malformed XML scenarios."""
    
    def test_llm_generated_malformed_xml(self):
        ...
        opts = ParseOptions(recovery_strategy=RecoveryStrategy.AGGRESSIVE, repair_attributes=True)
    ...
    def test_html_like_structures(self):
    ...
    def test_mixed_content_types(self):
        ...
        event_types = {type(e).__name__ for e in events}
        expected_types = {"StartElement", "EndElement", "Text", "Comment", "ProcessingInstruction"}
...
class TestIntegration:
    ...
    def test_full_pipeline_wellformed(self):
    ...
    def test_full_pipeline_malformed(self):
    ...
    def test_custom_options_integration(self):
...
class TestFixtures:
    ...
    def test_sample_xml_fixture(self, sample_xml):
    ...
    def test_malformed_xml_fixture(self, malformed_xml):
...
class TestParametrized:
    ...
    @pytest.mark.parametrize("recovery_strategy", [
        RecoveryStrategy.STRICT,
        RecoveryStrategy.LENIENT,
        RecoveryStrategy.AGGRESSIVE
    ])
    def test_recovery_strategies_parametrized(self, recovery_strategy):
    ...
    def test_entity_resolution_parametrized(self, entity, expected):
    ...
    @pytest.mark.parametrize("malformed_attr", [
    ...
    def test_malformed_attributes_parametrized(self, malformed_attr):
        """Test various malformed attribute scenarios."""
        xml = f'<root {malformed_attr}>content</root>'
        opts = ParseOptions(repair_attributes=True, recovery_strategy=RecoveryStrategy.AGGRESSIVE)
